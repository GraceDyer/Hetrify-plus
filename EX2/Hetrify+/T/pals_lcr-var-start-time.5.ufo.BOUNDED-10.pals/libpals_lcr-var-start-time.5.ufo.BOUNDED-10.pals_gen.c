...
int check(){
    ...
    cpu_t *c=&cpu;
    char stack[48];
    c->gp_regs[sp]=(uint64_t)&stack[48];
L_10000: {//addi_check
...
L_1008c: {//sw_check
    ...
    uint64_t rs2 = c->gp_regs[15];
    uint64_t mask32 = 0xFFFFFFFFULL << (4 * 8);
    uint64_t val32  = (uint64_t)(uint32_t)rs2 << (4 * 8);
    stack0[2]  = (stack0[2] & ~mask32) | val32;
    }
L_10090: {//jal_check
    goto L_10114;
    }
...
L_10114: {//lw_check
    uint64_t rs1 = c->gp_regs[8];
    c->gp_regs[15] = (int32_t)((stack0[2] >>(4*8))& 0xFFFFFFFF);
    }
L_10118: {//addi_check
    uint64_t rs1 = c->gp_regs[15];
